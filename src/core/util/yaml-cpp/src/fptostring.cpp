 ...
 * @param width     - minimum number of digits, fill with '0' to the left. Must be equal or smaller
 ...
 */
template <typename T> std::string FpToString(T v, int precision = 0)
{
    ...
    std::array<char, 28> output_buffer; // max digits of size_t plus sign, a dot and 2 letters for
                                        // 'e+' or 'e-' and 4 letters for the exponent
    ...
    if (exponent >= upperExponentThreshold || exponent <= lowerExponentThreshold) {
        ...
        *(output_ptr++) = (exponent >= 0) ? '+' : '-';
        ...
        auto exp_digits_ct =
            ConvertToChars(exp_digits.data(), exp_digits.data() + exp_digits.size(),
                           std::abs(exponent), /*.precision=*/2);
