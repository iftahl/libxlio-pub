...
template <typename To, typename From> JKJ_CONSTEXPR20 To bit_cast(const From &from)
{
#if JKJ_HAS_BIT_CAST
...
template <class Format, class CarrierUInt, class ExponentInt = int> struct ieee754_binary_traits {
    ...
    {
        return (u &
                ((carrier_uint(1) << (format::significand_bits + format::exponent_bits)) - 1u)) !=
            0;
    }
...
// integers.
JKJ_SAFEBUFFERS inline JKJ_CONSTEXPR20 stdr::uint_least64_t umul128_upper64(
...
template <stdr::size_t tier> struct floor_log10_pow2_info;
template <> struct floor_log10_pow2_info<0> {
...
template <> struct floor_log10_pow2_info<1> {
    using default_return_type = stdr::int_fast8_t;
...
template <> struct floor_log10_pow2_info<2> {
...
template <stdr::size_t tier> struct floor_log2_pow10_info;
template <> struct floor_log2_pow10_info<0> {
    using default_return_type = stdr::int_fast8_t;
...
template <> struct floor_log2_pow10_info<1> {
    using default_return_type = stdr::int_fast16_t;
    // 24-bits are enough in fact.
...
template <> struct floor_log2_pow10_info<2> {
    using default_return_type = stdr::int_fast16_t;
...
template <stdr::size_t tier> struct floor_log10_pow2_minus_log10_4_over_3_info;
template <> struct floor_log10_pow2_minus_log10_4_over_3_info<0> {
    using default_return_type = stdr::int_fast8_t;
    static constexpr stdr::int_fast16_t multiply = 77;
...
template <> struct floor_log10_pow2_minus_log10_4_over_3_info<1> {
    using default_return_type = stdr::int_fast8_t;
    // 24-bits are enough in fact.
...
template <> struct floor_log10_pow2_minus_log10_4_over_3_info<2> {
    using default_return_type = stdr::int_fast16_t;
...
template <stdr::size_t tier> struct floor_log5_pow2_info;
template <> struct floor_log5_pow2_info<0> {
...
template <stdr::size_t tier> struct floor_log5_pow2_minus_log5_3_info;
template <> struct floor_log5_pow2_minus_log5_3_info<0> {
    using default_return_type = stdr::int_fast32_t;
...
template <class Dummy> struct cache_holder<ieee754_binary32, Dummy> {
    ...
    static constexpr detail::array<cache_entry_type, detail::stdr::size_t(max_k - min_k + 1)> cache
        JKJ_STATIC_DATA_SECTION = {{UINT64_C(0x81ceb32c4b43fcf5), UINT64_C(0xa2425ff75e14fc32),
...
JKJ_INLINE_VARIABLE struct remove_t {
        ...
        on_trailing_zeros(DecimalSignificand significand, DecimalExponentType exponent) noexcept
    {
        remove_trailing_zeros_traits<remove_t, Format, DecimalSignificand,
                                     DecimalExponentType>::remove_trailing_zeros(significand,
                                                                                 exponent);
        return {significand, exponent};
    }
    ...
    using remainder_type = typename detail::stdr::conditional<
        ...
        typename detail::stdr::conditional<
            ...
            typename detail::stdr::conditional<
                ...
                typename detail::stdr::conditional<
                    upper_bound <=
                        detail::stdr::numeric_limits<detail::stdr::uint_least64_t>::max(),
    ...
    using decimal_exponent_type = typename detail::stdr::conditional<
        ...
        typename detail::stdr::conditional<
            ...
            typename detail::stdr::conditional<
                ...
                detail::stdr::int_least32_t, typename FormatTraits::exponent_int>::type>::type>::
        type;
...
struct remove_trailing_zeros_traits<policy::trailing_zero::remove_t, ieee754_binary32,
        ...
        detail::stdr::uint_least32_t &significand, DecimalExponentType &exponent) noexcept
    {
        ...
        r = detail::bits::rotr<32>(detail::stdr::uint_least32_t(significand * UINT32_C(42949673)),
                                   2);
        b = r < UINT32_C(42949673);
        s = s * 2 + b;
        significand = b ? r : significand;

        r = detail::bits::rotr<32>(detail::stdr::uint_least32_t(significand * UINT32_C(1288490189)),
                                   1);
...
template <class FormatTraits> struct impl : private FormatTraits::format {
    ...
    static constexpr int case_shorter_interval_left_endpoint_upper_threshold =
        2 +
        log::floor_log2(
            compute_power<count_factors<5>((carrier_uint(1) << (significand_bits + 2)) - 1) + 1>(
                10) /
            3);
    ...
    static constexpr int case_shorter_interval_right_endpoint_upper_threshold =
        2 +
        log::floor_log2(
            compute_power<count_factors<5>((carrier_uint(1) << (significand_bits + 1)) + 1) + 1>(
                10) /
            3);
               ...
               typename FormatTraits::exponent_int exponent_bits) noexcept
    {
        ...
        JKJ_IF_CONSTEXPR(tag == policy::decimal_to_binary_rounding::tag_t::to_nearest)
        {
            return impl<FormatTraits>::template compute_nearest<
                ...
                binary_to_decimal_rounding_policy, cache_policy, preferred_integer_types_policy>(
                s, exponent_bits);
        }
        else JKJ_IF_CONSTEXPR(tag ==
                              policy::decimal_to_binary_rounding::tag_t::left_closed_directed)
        {
