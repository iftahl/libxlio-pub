...
private:
    template <typename T> void _Set(Setting<T> &fmt, T value, FmtScope::value scope);
...
template <typename T> void EmitterState::_Set(Setting<T> &fmt, T value, FmtScope::value scope)
{
    switch (scope) {
    case FmtScope::Local:
