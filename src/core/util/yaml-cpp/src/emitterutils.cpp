...
bool GetNextCodePointAndAdvance(int &codePoint, std::string::const_iterator &first,
...
bool IsValidPlainScalar(const std::string &str, FlowType::value flowType, bool allowOnlyAscii)
{
    ...
    // check the start
    const RegEx &start =
        (flowType == FlowType::Flow ? Exp::PlainScalarInFlow() : Exp::PlainScalar());
    ...
    static const RegEx disallowed_flow = Exp::EndScalarInFlow() |
        (Exp::BlankOrBreak() + Exp::Comment()) | Exp::NotPrintable() | Exp::Utf8_ByteOrderMark() |
        Exp::Break() | Exp::Tab() | Exp::Ampersand();
    static const RegEx disallowed_block = Exp::EndScalar() |
        (Exp::BlankOrBreak() + Exp::Comment()) | Exp::NotPrintable() | Exp::Utf8_ByteOrderMark() |
        Exp::Break() | Exp::Tab() | Exp::Ampersand();
    const RegEx &disallowed = flowType == FlowType::Flow ? disallowed_flow : disallowed_block;
...
bool IsValidSingleQuotedScalar(const std::string &str, bool escapeNonAscii)
{
    ...
    return std::none_of(str.begin(), str.end(), [=](char ch) {
        return (escapeNonAscii && (0x80 <= static_cast<unsigned char>(ch))) || (ch == '\n');
    });
}
...
StringFormat::value ComputeStringFormat(const std::string &str, EMITTER_MANIP strFormat,
...
bool WriteLiteralString(ostream_wrapper &out, const std::string &str, std::size_t indent)
{
    out << "|\n";
    int codePoint;
    for (std::string::const_iterator i = str.begin();
...
bool WriteComment(ostream_wrapper &out, const std::string &str, std::size_t postCommentIndent)
{
...
{
    WriteDoubleQuotedString(out, EncodeBase64(binary.data(), binary.size()), StringEscaping::None);
    return true;
}
} // namespace Utils
} // namespace YAML
