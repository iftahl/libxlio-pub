...
template <typename V> struct node_iterator_value : public std::pair<V *, V *> {
...
template <typename V> struct node_iterator_type {
    using seq = node_seq::iterator;
    using map = node_map::iterator;
};

template <typename V> struct node_iterator_type<const V> {
...
private:
    ...
    struct proxy {
        explicit proxy(const node_iterator_value<V> &x)
            : m_ref(x)
        {
        }
...
public:
    ...
    template <typename W> bool operator==(const node_iterator_base<W> &rhs) const
    {
