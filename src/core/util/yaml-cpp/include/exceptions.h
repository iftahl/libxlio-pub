...
template <typename T>
inline const std::string KEY_NOT_FOUND_WITH_KEY(const T &,
...
template <typename T>
inline const std::string KEY_NOT_FOUND_WITH_KEY(const T &key,
...
template <typename T>
inline const std::string BAD_SUBSCRIPT_WITH_KEY(const T &key,
...
private:
    static const std::string build_what(const Mark &mark, const std::string &msg)
    {
        ...
        output << "yaml-cpp: error at line " << mark.line + 1 << ", column " << mark.column + 1
               << ": " << msg;
        return output.str();
    }
};
...
template <typename T> class YAML_CPP_API TypedKeyNotFound : public KeyNotFound {
public:
    TypedKeyNotFound(const Mark &mark_, const T &key_)
        : KeyNotFound(mark_, key_)
        , key(key_)
    {
    }
...
public:
    InvalidNode(const std::string &key)
        : RepresentationException(Mark::null_mark(), ErrorMsg::INVALID_NODE_WITH_KEY(key))
    {
    }
...
template <typename T> class TypedBadConversion : public BadConversion {
public:
    explicit TypedBadConversion(const Mark &mark_)
        : BadConversion(mark_)
    {
    }
};
...
public:
    explicit BadFile(const std::string &filename)
        : Exception(Mark::null_mark(), std::string(ErrorMsg::BAD_FILE) + ": " + filename)
    {
    }
