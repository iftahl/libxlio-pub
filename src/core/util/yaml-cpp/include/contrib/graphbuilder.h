...
public:
    ...
    virtual void *NewScalar(const Mark &mark, const std::string &tag, void *pParentNode,
...
// appropriate.
template <class Impl> class GraphBuilder : public GraphBuilderInterface {
public:
...
private:
    ...
    template <class T, class U> static T *CheckType(U *p) { return p; }
...
template <class Impl> typename Impl::Node *BuildGraphOfNextDocument(Parser &parser, Impl &impl)
{
    ...
    return static_cast<typename Impl::Node *>(BuildGraphOfNextDocument(parser, graphBuilder));
}
} //namespace YAML
